<script>
  ...
  const backgroundReel = $derived.by(() => {
    const separator = dop.reelUrl.includes("?") ? "&" : "?";
    return `${dop.reelUrl}${separator}${reelParams}`;
  });
</script>

<svelte:head>
  <title>{dop.name} | {data.content.site.name}</title>
</svelte:head>
...
<section class="section profile-stage">
  <div class="profile-bg">
    <iframe
      src={backgroundReel}
      title={`${dop.name} background reel`}
      frameborder="0"
  ...
  <div class="container profile profile-content">
    <div class="profile-intro profile-block-offset">
      <div class="profile-header">
        <h1 class="profile-name">{dop.name}</h1>
        <div class="profile-role">Director of Photography - The Founders</div>
      </div>
      <p>{dop.bio}</p>
    </div>
